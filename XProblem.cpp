...
{	
    ...
    fBoundary = (TSphereBoundary *)(new TSphereBoundary())
        ...
        ->Initialize();
	/*
	*fBeginBoundaryPosition = new double [3];
	for(int i = 0; i<3;i++)
	{
		fBeginBoundaryPosition[i] = new double [fBoundary->height_nodes * fBoundary->radius_nodes];
	}
/*
	for (int cr = 0; cr < fBoundary->height_nodes; cr++)
	{
		for (int i = 0; i <  fBoundary->radius_nodes; i++)
		{
			fBeginBoundaryPosition[0][cr*fBoundary->radius_nodes + i] = fBoundary->nodes[cr*fBoundary->radius_nodes + i]->x;
			fBeginBoundaryPosition[1][cr*fBoundary->radius_nodes + i] = fBoundary->nodes[cr*fBoundary->radius_nodes + i]->y;
			fBeginBoundaryPosition[2][cr*fBoundary->radius_nodes + i] = fBoundary->nodes[cr*fBoundary->radius_nodes + i]->z;
		}
	}//*/
...
void TImmersedBoundaryProblem::OutputBoundary(int timeStepNumber)
{
	...
	if (timeStepNumber % TProblem::GlobalSaveStep == 0 || timeStepNumber == 1)
	{
		...
		for (int cr = 0; cr < Circles; ++cr)
			for (int i = 0; i < InCircle; ++i)
			{
				//printf("debug %f %f %f \n", fBeginBoundaryPosition[0][cr*InCircle + i] , fBeginBoundaryPosition[1][cr*InCircle + i] , fBeginBoundaryPosition[2][cr*InCircle + i] );		
				//std::cout>>fBeginBoundaryPosition[0][cr*InCircle + i]>>" ">>fBeginBoundaryPosition[1][cr*InCircle + i]>>" ">>fBeginBoundaryPosition[2][cr*InCircle + i] >>endl;		
